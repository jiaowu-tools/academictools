...
(function () {
  ...
  function parseSmartMeetingText(text, options = {}) {
    ...
    const explicitEndMinutes = parseSmartMeetingEndMinutes(raw, startMinutes);
    const durationMinutes = Number.isFinite(explicitEndMinutes)
      ? explicitEndMinutes - startMinutes
      : parseSmartMeetingDurationMinutes(raw) || getDefaultSmartMeetingDurationMinutes();
    ...
    const meetingMode = parseSmartMeetingMode(raw);
    const campusInfo = parseSmartMeetingCampus(raw);
    const meetingClassroomPrefixes = getSmartMeetingClassroomPrefixes(raw, meetingMode, campusInfo);
    ...
    return buildSmartMeetingDraft({
      sourceKind: dateRange ? 'smart-date-range-meeting-text' : 'smart-meeting-text',
      date,
  ...
  function parseSmartMeetingDateRange(text, now = new Date()) {
    ...
    while ((match = pattern.exec(source))) {
      ...
      const endYear = match[4]
        ? Number(match[4])
        : (endMonth < startMonth || (endMonth === startMonth && endDay < startDay) ? startYear + 1 : startYear);
      const startDate = makeDateInput(startYear, startMonth, startDay);
      const endDate = makeDateInput(endYear, endMonth, endDay);
  ...
  function extractSmartMeetingTeacherBeforeDetailsSegment(text) {
    const source = String(text || '');
    const match = source.match(/(?:^|[\n\r,，;；。\s])\s*([\u4e00-\u9fa5A-Za-z]{2,8})(?:老师)?\s*(?=[,，;；]\s*(?:线上|线下|虚拟校区|虚拟教室|校区|教室))/u);
    return match ? match[1] : '';
  }
  ...
  function parseSmartMeetingMode(text) {
    ...
    if (/线上|在线|腾讯会议|钉钉|飞书|直播|远程|虚拟校区|虚拟教室/.test(normalized)) return 'online';
    return 'offline';
  }
  ...
  function getSmartMeetingClassroomPrefixes(text, meetingMode, campusInfo) {
    ...
    if (campusInfo.campus === '虚拟校区') return ['虚拟教室'];
  ...
  function parseSmartMeetingName(rawText, compactText) {
    ...
    if (scheduleRestrictionName) return scheduleRestrictionName;
  ...
  function parseSmartMeetingScheduleRestrictionName(text) {
    ...
    return /^(?:上午|下午|晚上|晚间|中午|早上)?\d{1,2}[:：]\d{2}\s*(?:以后|之后|起|开始)\s*不排课$/u.test(firstLine)
      ? firstLine
      : '';
  }
  ...
  function cutSmartMeetingNameBeforeDetails(text) {
    const source = String(text || '');
    const cutPatterns = [
      /(?:(?:\d{4})年)?\d{1,2}(?:月|[\/.-])\d{1,2}\s*(?:至|到|-|~|～)\s*(?:(?:\d{4})年)?\d{1,2}(?:月|[\/.-])\d{1,2}/u,
  ...
  function assertSelfTestSmartMeetingTextParsing() {
    ...
    const noClassAfterTimeDraft = parseSmartMeetingText([
      '15:00以后不排课',
      '8.8',
      '15:00-18:10',
      '虚拟校区',
    ...
    const noClassAfterTimeExpected = {
      date: '2026-08-08',
      startTime: '15:00',
      endTime: '18:10',
      durationMinutes: 190,
      ...
      meetingMode: 'online',
      meetingCampus: '虚拟校区',
      meetingName: '15:00以后不排课',
      teachers: '郑菲'
    };
    if (!noClassAfterTimeDraft.ok) throw new Error(`15点后不排课原文回归识别失败：${noClassAfterTimeDraft.message}`);
    Object.entries(noClassAfterTimeExpected).forEach(([key, value]) => {
      const actual = key === 'teachers' ? noClassAfterTimeDraft.teachers.join(',') : noClassAfterTimeDraft[key];
      if (actual !== value) throw new Error(`15点后不排课原文回归 ${key} 应为 ${value}，实际：${actual}`);
    });
    if (getMeetingDraftClassroomPrefixes(noClassAfterTimeDraft).join(',') !== '虚拟教室') {
      throw new Error(`15点后不排课原文回归教室应为虚拟教室，实际：${getMeetingDraftClassroomPrefixes(noClassAfterTimeDraft).join(',')}`);
    }
    ...
    const dateRangeVirtualExpected = {
      date: '2026-08-01',
      endDate: '2026-09-05',
      startTime: '08:30',
      endTime: '18:10',
    ...
    if (!dateRangeVirtualMeeting.ok) throw new Error(`预排会议原文回归识别失败：${dateRangeVirtualMeeting.message}`);
    Object.entries(dateRangeVirtualExpected).forEach(([key, value]) => {
      const actual = key === 'teachers' ? dateRangeVirtualMeeting.teachers.join(',') : dateRangeVirtualMeeting[key];
      if (actual !== value) throw new Error(`预排会议原文回归 ${key} 应为 ${value}，实际：${actual}`);
    });
    if (getMeetingDraftClassroomPrefixes(dateRangeVirtualMeeting).join(',') !== '虚拟教室') {
      throw new Error(`预排会议原文回归教室应为虚拟教室，实际：${getMeetingDraftClassroomPrefixes(dateRangeVirtualMeeting).join(',')}`);
    }
